...
def evaluation_model(model, X_test:pd.DataFrame, y_test:pd.Series) -> dict:
    try:
        ...
        with Live(save_dvc_exp = True) as dvc_live:
            dvc_live.log_metric("accuracy", acc) 
            dvc_live.log_metric("precision", pre) 
            dvc_live.log_metric("recall", recall) 
            dvc_live.log_metric("f1_score", f1score) 

            dvc_live.log_param("test_size", test_size)
            dvc_live.log_param("n_estimators", n_estimators)
